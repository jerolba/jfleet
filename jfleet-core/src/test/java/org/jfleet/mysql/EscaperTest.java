...
import org.jfleet.mysql.Escaper;
...
public class EscaperTest {
	...
	public void testEscapeEscapedByChar() {
		...
		String escaped = escaper.escapeForLoadFile(text);
		assertEquals("Some text with some "+ESCAPED_BY_CHAR+ESCAPED_BY_CHAR+" escape char "+ESCAPED_BY_CHAR+ESCAPED_BY_CHAR+ESCAPED_BY_CHAR+ESCAPED_BY_CHAR+" to change", escaped);
		logger.info("FROM:");
		logger.info(text);
		logger.info("TO:");
		logger.info(escaped);
	...
	public void testEscapeLineTermination() {
		...
		String escaped = escaper.escapeForLoadFile(text);
		assertEquals("Some text with "+ESCAPED_BY_CHAR+LINE_TERMINATED_CHAR+"more than one line"+ESCAPED_BY_CHAR+LINE_TERMINATED_CHAR, escaped);
		logger.info("FROM:");
		logger.info(text);
		logger.info("TO:");
		logger.info(escaped);
	...
	public void testEscapeFieldTermination() {
		...
		String escaped = escaper.escapeForLoadFile(text);
		assertEquals("1234"+ESCAPED_BY_CHAR+FIELD_TERMINATED_CHAR+"value"+ESCAPED_BY_CHAR+FIELD_TERMINATED_CHAR+"5678"+ESCAPED_BY_CHAR+FIELD_TERMINATED_CHAR, escaped);
		logger.info("FROM:");
		logger.info(text);
		logger.info("TO:");
		logger.info(escaped);
